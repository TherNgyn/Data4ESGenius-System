...
def calculate_esg_score(df):
    ...
    valid_scores = [
        score
        for score in [result_df["E_score"], result_df["S_score"], result_df["G_score"]]
...
def get_model():
    try:
        ...
        st.warning(
            "Không tìm thấy mô hình đã lưu. Bạn cần upload dữ liệu để huấn luyện mô hình mới."
        )
        model = None
        scaler = None
        feature_order = None
...
def display_results(results):
    ...
    if score_range == "First Quartile":
        color = "red"
        interpretation = (
            "Hiệu suất ESG kém. Cần cải thiện đáng kể trong hầu hết các lĩnh vực."
        )
        ...
        color = "lightgreen"
        interpretation = (
            "Hiệu suất ESG tốt. Đang đi đúng hướng với nhiều lĩnh vực đạt hiệu quả cao."
        )
    else:
        color = "green"
        interpretation = (
            "Hiệu suất ESG xuất sắc. Đang dẫn đầu trong phần lớn các lĩnh vực ESG."
        )

    st.markdown(
        f"### Phân loại ESG: <span style='color:{color}'>{score_range}</span>",
        unsafe_allow_html=True,
    )
    ...
    scores = [
        results["E_score"].values[0] * 10,
        results["S_score"].values[0] * 10,
        results["G_score"].values[0] * 10,
    ]
    ...
    weakest_pillar = min(
        [
            ("Environmental", results["E_score"].values[0]),
...
if evaluation_mode == "Nhập dữ liệu thủ công":
    ...
    for i, indicator in enumerate(environmental_indicators):
        ...
        def update_value(indicator_name):
            def callback():
                st.session_state.all_values[indicator_name] = st.session_state[
                    f"env_{indicator_name}"
                ]
    ...
    if st.button("Tính điểm ESG", key="manual_calc_button"):
        ...
        if model is not None and scaler and feature_order is not None:
            try:
                ...
                missing_features = [
                    f for f in required_features if f not in input_df.columns
                ]
                if missing_features:
                    st.warning(
                        f"Thiếu {len(missing_features)} cột: {', '.join(missing_features[:5])}..."
                    )
                ...
                proba_df = pd.DataFrame([predicted_proba[0]], columns=model.classes_)
            ...
            if st.button("Phân tích ESG"):
                ...
                if model is not None and scaler is not None:
                    ...
                    if feature_order is not None:
                        ...
                        aligned_data = data.reindex(
                            columns=feature_order, fill_value=50.0
                        )
                    else:
                        # Fallback nếu không có file đặc trưng
                        aligned_data = data[feature_cols].fillna(
                ...
                if len(results) > 1:
                    ...
                    result_table = results[
                        ["E_score", "S_score", "G_score", "ESG_score", "score_range"]
                    ]
                    ...
                    if has_time_col:
                        ...
                        ax.plot(
                            results.index, results["S_score"] * 10, "b-", label="Social"
                        )
                        ax.plot(
                            results.index,
                            results["G_score"] * 10,
                        ...
                        ax.plot(
                            results.index,
                            results["ESG_score"] / 10,
                            "k--",
                            label="ESG Overall (÷10)",
                        )
                        ax.legend()
                        ...
                        colors = {
                            "First Quartile": "red",
                            "Second Quartile": "orange",
                            "Third Quartile": "lightgreen",
                            "Fourth Quartile": "darkgreen",
                        }
                        ...
                        for score_range in results["score_range"].unique():
                            if pd.notna(score_range):
                                ...
                                ax.bar(
                                    results.index[mask],
                                    results["ESG_score"][mask],
                                    color=colors.get(score_range, "gray"),
                                    label=score_range,
                                )
        ...
        try:
            ...
            if st.button("Train mô hình"):
                ...
                original_columns = [
                    ...
                    if not col.endswith("_percentile")
                    and col
                    not in ["E_score", "S_score", "G_score", "ESG_score", "score_range"]
                ]

                X = esg_scored_data[original_columns].fillna(
                    esg_scored_data[original_columns].mean()
                )
                ...
                if len(y) == 0:
                    ...
                    with open("esg_model.pkl", "rb") as f:
                        st.download_button(
                            "Tải xuống mô hình", f, file_name="esg_model.pkl"
                        )

                    with open("esg_scaler.pkl", "rb") as f:
                        st.download_button(
                            "Tải xuống scaler", f, file_name="esg_scaler.pkl"
                        )
